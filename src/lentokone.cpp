...
void lentokone::tarkista_suunta_kohteeseen() {
	if (kohde.x > 0 && kohde.y > 0) {
		apuvalineet::vektori vektori_kohteeseen = apuvalineet::suunta_vektori(paikka, kohde);
...
void lentokone::ota_selvitys(std::string tmp, int toiminto) {
	...
	if (toiminto == Peli::SUUNTA) {
		...
		if (kohde.x == 0 && kohde.y == 0 && reitti.size()) {
			kohde = anna_piste().paikka;
		}
